...
;
define('addMorePage',["jquery","underscore","dealPrice"],function($,_,dealPrice){
	var element={};
	element.dealParam=function(obj){
		...
		this.distance=obj.distance|| 320;
		...
		this.endCallback= obj.endCallback;
        this.dealMethod=obj.dealMethod||{}; //方法
        this.longDate='';
        this.noDataClass=obj.noDataClass;
        this.hasNext = false;
	}
	...
	element.render=function(obj){
		var self = this;
		if (self.eventName == "scroll"){
			var winHeight= $("body").outerHeight();
			var scrollTop = $(window).scrollTop();
			var viewHeight= $(window).height();
			if (scrollTop + viewHeight > winHeight-self.distance && self.hasNext&&!self.flag ){
				self.flag =true;
				self.hasNext = false;
				self.getData({storeDateLong:self.longDate});
			}
		}
	}
	...
	element.getData=function(url_param){
		var self=this;
		var dataUrl=self.url+'&pageSize='+self.pageSize;
		
		$.get(dataUrl,url_param,function(data){
			var dataDetail = data.data;
			var multiData=$.extend({},{data:dataDetail},self.dealMethod);
			var iLength = dataDetail.storeUpList.length;
			if(self.firstTime){ //第一次请求
				if(iLength == 0){
					$(self.container).html('<div class="collect_no_data '+self.noDataClass+'"></div>')
				}else{
					self.hasNext = dataDetail.hasNext;
					$(self.container).html(self.templ(multiData));
					if(self.hasNext == true){
						self.longDate = dataDetail.storeUpList[iLength-1].storeDateLong;
					}
				}
	...
	var addMorePage=function(){}
	addMorePage.prototype=element;

	return addMorePage;
});
require(["jquery","underscore","publicTabModule","ddbase","reading_center_nav","publicMethod","addMorePage","loginModule"],function($,_,publicTabModule,ddbase,leftNav,publicMethod,addMorePage,loginModule){
   
	 publicMethod.init();
	//左侧菜单
	
	leftNav.init('#myCollectNav');
	...
	var collect = {
		data : {
			baseUrl:"/media/api2.go?action=dDReaderStoreUpList&selfType=0&type=",
			book:"media",
			article :"article",
			post : "post",
			tabStatus :"book"
		...
		init:function(){
			...
			if(self.data.tabStatus == "book"){
				tabPara = self.data.book;
				template = '#collectBook';
				container = '.collect_book_con .inner';

				finalUrl = self.data.baseUrl+tabPara+ddbase.baseApiParams;
				if(!self.newData.bNew1){
					self.newData.bNew1 = new addMorePage();
					self.addMoreSetData(template,container,finalUrl,self.newData.bNew1,'');
				}
				self.newData.bNew2&&(self.newData.bNew2.isShow = false);
				self.newData.bNew3&&(self.newData.bNew3.isShow = false);
				self.newData.bNew1.isShow = true;
			}else if(self.data.tabStatus == "article"){
				tabPara = self.data.article;
				template = '#collectArticle';
				container = '.collect_article_con';

				finalUrl = self.data.baseUrl+tabPara+ddbase.baseApiParams;
				if(!self.newData.bNew2){
					self.newData.bNew2 = new addMorePage();
					self.addMoreSetData(template,container,finalUrl,self.newData.bNew2,'article_no_data');
				}
				self.newData.bNew1&&(self.newData.bNew1.isShow = false);
				self.newData.bNew3&&(self.newData.bNew3.isShow = false);
				self.newData.bNew2.isShow = true;
			}else if(self.data.tabStatus == "post"){
				tabPara = self.data.post;
				template = '#collectPost';
				container = '.collect_post_con';

				finalUrl = self.data.baseUrl+tabPara+ddbase.baseApiParams;
				if(!self.newData.bNew3){
					self.newData.bNew3 = new addMorePage();
					self.addMoreSetData(template,container,finalUrl,self.newData.bNew3,'post_no_data');
				}
				self.newData.bNew1&&(self.newData.bNew1.isShow = false);
				self.newData.bNew2&&(self.newData.bNew2.isShow = false);
				self.newData.bNew3.isShow = true;
			}

		},
		addMoreSetData:function(tem,con,finalUrl,obj,noDataClass){
			var self = this;
			if(tem == '#collectBook'){
				endCon = '.collect_book_con';
			}else{
				endCon = con;
			}
		    if(obj != false){
		    	obj.init(
					{
				        ...
				        eventName:'scroll',
				        url:finalUrl,
				        dealMethod:{dealTime:ddbase.dealTime,dealLink:ddbase.getBookUrl},
				        noDataClass:noDataClass,
				        endCallback:function(){
				            $(".loading").remove();
				            $(endCon).append($('<div class="endload">加载完毕!</div>'))
				        }
					}
				)
		    }
			...
			$(window).scroll(function(){
		        obj&&obj.isShow&&obj.render();
		    });
		}
	}

	collect.init();
	publicTabModule.slideTab($('.index_subnav_module'),'only_one','click',$('#myCollect .content'),function(){
		var self = $(this);
		//每点一次执行一次
		collect.data.tabStatus = self.attr('status');
		collect.init();
		
	});
	
});
define("myCollect", function(){});
