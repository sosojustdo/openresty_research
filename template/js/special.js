...
require(['jquery','underscore','ddbase',"publicMethod","dealPrice","add_more"], function ($,_,ddbase,publicMethod,dealPrice,addMore) {
    ...
    var special={
        ...
        getUrl:{
            specialMesUrl:'/media/api.go?action=specialtopic&channelType=DDDS_All&needcolumn=1&columnNuber=0&stId='+encodeURIComponent(ddbase.getQueryString("stId"))+ddbase.baseApiParams,
            specialBookUrl:''
        },
        setSpecialMes:function(data){
            var codeStatus=data.status.code;
            if(codeStatus===0){
                var dataObj=data.data;
                $(".bags i").html(dataObj.blockVoList[0].blockName)
                special.getUrl.specialBookUrl='/media/api.go?action=column&columnType=all_'+dataObj.blockVoList[0].relationColumnCode+'&isFull=1&returnType=json'+ddbase.baseApiParams
                var specialMes=$(".special .specialmes")
                var specialMesTemplate=_.template($("#special_mes_module").html());
                specialMes.append(specialMesTemplate(dataObj));
                var addMore1=new addMore();
                addMore1.init({
                    template:"#morelist_smallcell_template",
                    container:".special .booklist",
                    startNumber:0,
                    endNumber:17,
                    gapNumber:18,
                    eventName:'scroll',
                    url:special.getUrl.specialBookUrl,
                    dealMethod:{dealPrice:dealPrice},
                    endCallback:function(){
                        $(".special .booklist").append("<div class='go_for_more'><a href='javascript:;'>加载完毕！</a></div>");
                    }
                });
        ...
        startGetData:function(){
            var _self=this;
            _self.getData("GET",_self.getUrl.specialMesUrl,"",_self.setSpecialMes,_self.getErr);
        }
    };
    special.startGetData();
});
