...
define('right_bars',['jquery','underscore','ddbase','loginModule','post'],function($,_,ddbase,loginModule,post) {
	var defaultSetting = {
			setting:{
				post: true,
				backToTop : true
	...
	var initEvents = function(){
		_.each(buttons,function(button){
			switch(button){
				case 'post':
					$('body').delegate('.right_bars button.post','click',function(){
						if(!ddbase.token()){
							loginModule();
							return;
						}
						var authory = $('#joinbarbtn').data('authory');
						if(authory == '4'){
							$('body').append('<div class="shadow"></div>')
							$('body').append(ddbase.template($('#joinbarWinTemplate').html()));
							$('body').delegate('.joinbarWin button.cannel','click',function(){
								$('.joinbarWin').remove();
								$('.shadow').remove();
							})
							$('body').delegate('.joinbarWin button.sure','click',function(){
								ddbase.joinBar({
						    		barId:defaultSetting.params.barId,
						    		actionType:1,
						    		custId:ddbase.getQueryString('custId'),
						    		callback:function(response){
						    			location.reload();
							...
							post.show({
								barId:defaultSetting.params.barId,
								callback: defaultSetting.params.callback || function(){}
							})
						}
					})
					break;
				case 'backToTop':
					$('body').delegate('.right_bars button.backToTop','click',function(){
						$(window).scrollTop(0);
					})
					break;
			}
	...
	var init = function(){
		for(var item in defaultSetting.setting){
			if(defaultSetting.setting[item]) buttons.push(item);
		}
		var html = ddbase.template($('#rightBarsTemplate').html(),{buttons:buttons});
		$('body').append(html);
		initEvents();
	}
	return {
		render: function(options){
			defaultSetting.setting = _.extend(defaultSetting.setting,options.setting);
			defaultSetting.params = options.params;
			init();
		}
	}
});
require(['jquery','underscore','publicMethod','ddbase','loginModule','bar_info','right_bars'], function ($,_,publicMethod,ddbase,loginModule,barInfo,rightBars) {
    publicMethod.init();

    var getRelativeBooks = function(){
	    $.get('/media/api.go?action=queryBarOneBook&barId='+barId+ddbase.baseApiParams,function(response){
	    	if(response.status.code != 0 || !response.data.books.length) return;
	    	var html = ddbase.template($('#relativeBookTemplate').html(),_.extend(response.data,{getBookUrl:ddbase.getBookUrl,barId:barId}));
	    	$('.relativeBooks').html(html);
	    })
    }
    var getRecommandbars = function(){
	    $.get('/media/api.go?action=queryRecommendBar&barId'+barId+ddbase.baseApiParams,function(response){
	    	if(response.status.code != 0) return;
	    	var html = ddbase.template($('#recommandbarsTemplate').html(),_.extend(response.data,{barId:barId}));
	    	$('.recommandbars').html(html);
	    })
    }
    var getPosts = function(){
        allowgetData = false;
        $('.barlist').append('<div class="loading"></div>');
        $.get('/media/api.go?action=queryArticleListV2&barId='+barId+ddbase.baseApiParams,{
            lastModifiedDateMsec:lastModifiedDateMsec
        },function(response){
            $('.barlist').find('.loading').remove();
            if(response.status.code != 0) return;
            if (response.data.articleNum == 0) {
                $('.barlist').append('<div class="overtip">您是第一个发帖的人，内容越精彩奖励越丰厚哦<div>');
                return;
            };
            var len = response.data.articleList.length;
            var html = ddbase.template($('#postTemplate').html(),_.extend(response.data,{newDealTime:ddbase.newDealTime}));
            $('.barlist').append(html);
            if (len === 0 || len == response.data.articleNum) {
                $('.barlist').append('<div class="overtip">没有数据可加载了，快去发帖吧~<div>');
                return;
            };
            lastModifiedDateMsec = response.data.articleList.slice(-1)[0].lastModifiedDateMsec;
            allowgetData = true;
        })
    }
    var barId = ddbase.getQueryString('barId');
    var lastModifiedDateMsec = null,allowgetData=true;
    if(barId){
    	//bar的相关信息
    	barInfo.render({
            el:'.barDes',
    ...
    //事件添加
    $('body').delegate('.subs-list a.join','click',function(){//推荐吧的加入
    	if(!ddbase.token()){
             loginModule();
             return;
        };
    	...
    	ddbase.joinBar({
    		...
    		callback:function(response){
    			...
    			target.removeClass('join').addClass('has-added').text('进入吧').attr('href','bar_detail_page.html?barId='+barId);
    		}
    	})
    });
    $('body').delegate('.relativeBooks #moreBooks','click',function(){//点击获取更多书籍
        var barName = $('.master-info .barName').attr('data-name');
        window.open('more_books_page.html?barId='+barId+'&barName='+barName,'_blank');
    })

    //滚动加载
    $(window).scroll(function () {
        if(ddbase.onTime(321) && allowgetData) getPosts();
    });

    //右侧工具栏
    rightBars.render({params:{
        barId:barId,
        // callback: getPosts
        callback: function(){
            window.location.href = window.location.href;
