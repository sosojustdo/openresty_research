...
require(['jquery',"publicMethod","add_more","ddbase"], function ($,publicMethod,addMore,ddbase) {
    publicMethod.init();
    var deviceType = ddbase.getQueryString("deviceType")||'YC_All';
    var pastSpecial = {
    	url: '/media/api.go?action=specialtopichistory&channelType=ALL&deviceType='+deviceType
    }
    
    var addMore=new addMore();
    addMore.init({
        template:"#past_special_template",
        container:".past_special",
        startNumber:0,
        endNumber:19,
        gapNumber:20,
        eventName:'scroll',
        url:pastSpecial.url+ddbase.setBaseApiParams,
        dealMethod:{},
        endCallback:function(){
            $(".morelistmes").remove();
            $(".past_special").append($('<div style="clear:both;width:106px;height:32px;margin:30px auto;" class="morelistmes">加载完毕!</div>'))
        }
