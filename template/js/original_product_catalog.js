...
define('original_product_catalog_left',["jquery","underscore","ddbase"],function($,_,ddbase){
	...
	element.init=function(obj){
		...
		$.get(obj.url+'&mediaId='+mediaId,function(data){
            var multiData=$.extend({},{data:data.data},{mediaId:mediaId});
...
define('original_product_copyright',["jquery","ddbase"],function($,ddbase){
	...
	element.init=function(obj){
		...
		$.get(obj.url+'&mediaId='+mediaId,function(data){
			...
			var data = data.data.mediaSale.mediaList[0];

			$(".copyright_content span").each(function(){
			 	$(this).html(data.title);
			 })
			$("#categorys").html(data.categorys);
        	$('.navigation_module').append('<a href="index_page.html">当当读书</a><a href="classification_list_page.html?category='+data.categoryIds+'&dimension=dd_sale&order=0">'+data.categorys+'</a><a class="focus">'+data.title+'</a>')
...
require(["publicHeaderNav","publicMethod","publicSideCodeModule","original_product_catalog_left","original_product_copyright","publicHeaderChildnavModule","publicHeadersearch"], function (publicHeaderNav,publicMethod,publicSideCodeModule,left,right,publicHeaderChildnavModule,publicHeadersearch) {
    publicMethod.init();
    //菜单下拉条
    publicHeaderNav.itHover();
    publicHeaderChildnavModule.itHover();
    ...
    left.init({
        "url":"/media/api2.go?action=getAllChapterByMediaId&deviceType=pconline&platformSource=DDDS-P&returnType=json&orderSource=30000",
        "template":"#catalog_menu",
        "container":"#original_catalog_show"
    })

    right.init({
        "url":"/media/api2.go?action=getMedia&deviceType=pconline&platformSource=DDDS-P"
    })
});
define("original_product_catalog", function(){});
