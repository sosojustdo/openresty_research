...
define('relatedBook',["jquery","underscore","ddbase","loginModule","signOutModule"],function($,_,ddbase,loginModule,signOutModule){
	var list={};

	list.init=function(obj){
		var param= ddbase.getQueryString(obj.param);
		var cIdName = ddbase.getQueryString(obj.cIdName);
		var url=obj.url+"&digestId="+param+"&ddChannelId="+cIdName;
		var token = ddbase.token();
		var templ= _.template($(obj.template).html());
		var self= this;
		$.get(url,function(data){
			if (!data.data.digestDetail.ebookInfo || !data.data.digestDetail.ebookInfo.bookName){
				$("#related_book").remove()
			}else{
				$(obj.container).html(templ(data.data));
			}
			
			// 处理文章名称、作者、标题等
			$("#article_content .title").text(data.data.digestDetail.cardTitle);
			$("#article_content .des span a").text(data.data.digestDetail.authorList[0].name)
			if(data.data.digestDetail && data.data.digestDetail.reviewCnt){
				$("#comment_number span").text("("+data.data.digestDetail.reviewCnt+")");
			}
			
			//在文章信息的模块处理文章的赞、收藏 
			if(data.data.digestDetail.isPraise && data.data.digestDetail.isPraise!=0){
				$(".sidebar_menu .favourite").attr("data-praise",true).addClass("selected");
			}else{
				$(".sidebar_menu .favourite").attr("data-praise",false)
			}
			if(data.data.digestDetail.alreayMark){
			...
			// 处理举报和删除文章
			var removeString='<div class="management"><a id="remove_post">删除</a></div>';
			var reportString='<div class="management"><a id="report_post">举报</a></div>';
			var custId=ddbase.getCustId();
			var	isOwner= data.data.channel.isOwner;
			if(ddbase.token() && isOwner){
				$("#article_content .title").before($(removeString));
			}else{
				$("#article_content .title").before($(reportString));
			}

			// 删除
			self.removeArticle(token,param,cIdName,isOwner)

		})
		// 举报
		this.reportArticle(param);
		this.singout();
	...
	// 举报文章
	list.reportArticle=function(param){
		$("#article_content").on("click","#report_post",function(){
            ...
            $("#confirm_report").click(function() {
                ...
                if (!flag) {
                    ...
                    var queryString = "&targetId=" + targetId + "&targetType=" + targetType + "&token=" + ddbase.token();
                    $.ajax({
                        type: "get",
                        url: reportUrl,
	...
	list.removeArticle=function(token,param,cIdName,isOwner){
		$("#article_content").on("click","#remove_post",function() {
            if (token&&isOwner) {
                ...
                $("#confirm_delete").click(function() {
                    ...
                    var deletArticleUrl = "/media/api2.go?action=deleteDigest" + ddbase.setBaseApiParams();
                    $.ajax({
                        type: "get",
                        url: deletArticleUrl,
                        data: obj,
                        ...
                        success: function(data) {
                            if (data.status.code == 0) {
                                ...
                                window.location.href = "channeldetail_page.html?cId=" + cIdName;
                            } else {
                                ddbase.setMessageTip(data.status.message)
                            }
                        },
...
define('articleComment',["jquery", "underscore", "ddbase", "loginModule"], function($, _, ddbase, loginModule) {
    var comment_list = {};
    ...
    comment_list.getData = function(url, flag) {
        ...
        $.get(url + "&pageNum=" + self.pageNum + "&commentId=" + self.lastCommentId, function(data) {
            ...
            $(".loading").remove()
            if (flag && flag != undefined) {
                $(self.container).html('').html(self.templ(multiData));
            } else {
...
require(['jquery','ddbase','publicMethod','hotInvitation','relatedBook','articleComment','articleDetailRightTools','channelInfo'], function ($,ddbase,publicMethod,hotInvitation,relatedBook,articleComment,tools,channelInfo) {
    publicMethod.init();
 //    sideBarCommunity.init({
	// 	param:"cId",
	// 	url:"/media/api2.go?action=channel"+ddbase.setBaseApiParams()
	// })
	var articalTextUrl="/media/api2.go?action=getDigestContentForH5"+ddbase.setBaseApiParams()+"&digestId="+ddbase.getQueryString("digestId");
	var articletype=ddbase.getQueryString("articletype");
	var targetSource;
	if (articletype==5){
		targetSource=7000;
	}
	...
	$.get(articalTextUrl,function(data){
		$("#article_content .detail").append(data)
		// 右侧工具栏
		tools.init({
	...
	relatedBook.init({
		url:"/media/api2.go?action=getDigestDetail"+ddbase.setBaseApiParams(),
		template:"#related_book_temp",
	...
	channelInfo.startGetData($(".master-info"));

});	
define("article_detail_page", function(){});
