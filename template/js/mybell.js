...
require(['jquery','underscore','ddbase',"publicMethod","reading_center_nav"], function ($,_,ddbase,publicMethod,leftNav) {
    publicMethod.init();

    leftNav.init(".mybellnav");

    function setBaseApiParams(){
        var channelId = ddbase.getQueryString('channelId');
        if(channelId == null){
            channelId = 70000;
        }
        ...
        return "&deviceSerialNo=html5&macAddr=html5&channelType=html5&permanentId="+ permanentId +"&returnType=json&channelId="+ channelId +"&clientVersionNo=5.0.0&platformSource=DDDS-P&fromPlatform=106&deviceType=Android&token="+ddbase.token();
    };
    var mybell={
        ...
        getUrl:{
            ...
            //mybellGoldeUrl:'/media/api2.go?action=getAccountInfo&accountType=master&returnType=json&deviceType=Android&token='+ddbase.getCookie("sessionID"),
            //mybellSilverUrl:'/media/api2.go?action=getAccountInfo&accountType=attach&returnType=json&deviceType=Android&token='+ddbase.getCookie("sessionID")
            mybellGoldeUrl:'/media/api2.go?action=getAccountInfo&accountType=master'+setBaseApiParams(),
            mybellSilverUrl:'/media/api2.go?action=getAccountInfo&accountType=attach'+setBaseApiParams()
        },
        setMybellGolde:function(data){
            var codeStatus=data.status.code;
            if(codeStatus===0){
                var dataObj=data.data;
                var bellBalanceWrap=$(".mybell_golden_balancewrap");
                var bellBalanceWrapTemplate= _.template($("#mybell_balance_module").html());
                bellBalanceWrap.append(bellBalanceWrapTemplate(dataObj));
                var mybellGoldeWrap=$(".mybell_golden_ticket")
                var mybellGoldeWrapTemplate=_.template($("#mybell_golden_module").html());
                mybellGoldeWrap.append(mybellGoldeWrapTemplate(dataObj));
                $(".mybellleft").css({"min-height":$(".mybellwrap").height()})
            }else{
                //数据接口出错提示
            }
        },
        setMybellSilver:function(data){
            var codeStatus=data.status.code;
            if(codeStatus===0){
                var dataObj=data.data;
                var bellBalanceWrap=$(".mybell_silver_balancewrap");
                var bellBalanceWrapTemplate= _.template($("#mybell_balance_module").html());
                bellBalanceWrap.append(bellBalanceWrapTemplate(dataObj));
                var mybellSilveWrap=$(".mybell_silver_ticket");
                var mybellSilveWrapTemplate=_.template($("#mybell_silver_module").html());
                mybellSilveWrap.append(mybellSilveWrapTemplate(dataObj));
                $(".mybellleft").css({"height":$(".mybellwrap").height()})
            }else{
                //数据接口出错提示
            }
        },
        ...
        tabChange:function(){
            var _self=this;
            $(".mybell_tab_module").on("click","span",function(){
                if($(this).text()==="金铃铛"){
                    $(".mybell_tab_module .bar").animate({"left":"0px"},100);
                    $(".mybell_silver_balancewrap").hide();
                    $(".mybell_silver_ticket").hide();
                    $(".mybell_golden_balancewrap").show();
                    $(".mybell_golden_ticket").show();
                }else{
                    $(".mybell_golden_balancewrap").hide();
                    $(".mybell_golden_ticket").hide();
                    $(".mybell_tab_module .bar").animate({"left":"88px"},100);
                    if($(".mybell_silver_balancewrap").html()===""){
                        _self.getData("post",_self.getUrl.mybellSilverUrl,"",_self.setMybellSilver,_self.getErr);
                    }else{
                        $(".mybell_silver_balancewrap").show();
                        $(".mybell_silver_ticket").show();
                    }

                }
                $(".mybell_tab_module span").removeClass("on");
                $(this).addClass("on");
            })
        },
        startGetData:function(){
            var _self=this;
            _self.tabChange();
            _self.getData("post",_self.getUrl.mybellGoldeUrl,"",_self.setMybellGolde,_self.getErr);

        }
    };
    mybell.startGetData();
});
define("mybell", function(){});
