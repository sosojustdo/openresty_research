...
define('getDepositShowView',["jquery",'ddbase',"underscore"],function ($,ddbase,_) {
	//获取用户充值选择界面数据
	var getDepositShowView = {};

	getDepositShowView.init = function(options){
		var self = this;
		self.paymentId = options.paymentId;
		self.fromPlatform = options.fromPlatform;
		self.el = options.el;
		self.template = options.template;

		$.ajax({
	        method:'POST',
	        url:'/media/api2.go?action=getDepositShowView&paymentId='+self.paymentId+'&fromPaltform=ds_android'+ddbase.setBaseApiParams().replace('fromPlatform=106','fromPlatform='+self.fromPlatform),
	        success:function(response){
		    	if(parseInt(response.status.code) == 0){
		    		var activityInfos = response.data.activityInfos;
		    		if(activityInfos.length > 0){
		    			$(self.el).html(_.template(self.template)({data:activityInfos}));
		    		}
		    	}
	        }
        }) 
...
define('wxPayForPC',["jquery","ddbase"],function ($,ddbase) {
	//微信PC扫码支付
	var wxPayForPC = function(option){
		//支付
		$.ajax({
	        method:'POST',
	        url:'/media/api2.go?action=wxPayForPC&orderId='+option.orderId+'&totalPrice='+option.totalPrice+'&isPaperBook='+option.isPaperBook+'&act='+option.act+ddbase.setBaseApiParams().replace('fromPlatform=106','fromPlatform='+option.fromPlatform),
	        success:function(response){
		    	if(parseInt(response.status.code) == 0){
		    		if(option.callback){
		    			option.callback();
...
define('thirdPayMethord',["jquery","ddbase","wxPayForPC","loginModule"],function ($,ddbase,wxPayForPC,loginModule) {
	//充值前先加购物车
	var thirdPayMethord = function(option){
		...
		var depositPayment = option.depositPayment; //weixin：1017；alipay:1018
		var act = option.act; //onekeybuy （一键支付）,prepaid（充值）
		var callback = null;
		if(option.callback){
		...
		$.ajax({
	        method:'POST',
	        url:'/media/api2.go?action=multiAction&field={"dependActions":[{"action":"appendCart","params":{"productIds":"'+productIds+'","oneKeyBuy":"true","referType":"personal","fromPlatform":"'+fromPlatform+'","orderSource":"'+orderSource+'"},"parseParams":{"cartId":"cartId"}},{"action":"getOrderFlow","params":{"productIds":"'+productIds+'","fromPlatform":"'+fromPlatform+'","orderSource":"'+orderSource+'"}},{"action":"savePayment","params":{"fromPlatform":"'+fromPlatform+'","orderSource":"'+orderSource+'"}},{"action":"submitOrder","params":{"isChargeOrder":"1","fromPlatform":"'+fromPlatform+'","isPaperBook":"'+isPaperBook+'","depositPayment":"'+depositPayment+'","orderSource":"'+orderSource+'"}}]}'+ddbase.setBaseApiParams().replace('fromPlatform=106','fromPlatform='+option.fromPlatform),
	        success:function(response){
		    	if(parseInt(response.status.code) == 0){
		    		...
		    		if(response.data.result.submitOrder.data.result){
		    			var result = response.data.result.submitOrder.data.result;
		    			if(depositPayment == '1017'){
		    				//微信
		    				wxPayForPC({isPaperBook:isPaperBook,act:act,fromPlatform:fromPlatform,orderId:result.order_id,totalPrice:result.total,callback:callback});
		    			}
		    			if(depositPayment == '1018'){
		    				//支付宝
		    				var transferUrl = 'alipay_page.html?orderId='+result.order_id+'&totalPrice='+result.total+'&act='+act;
...
require(["jquery","publicMethod","getDepositShowView","publicGetData","thirdPayMethord","ddbase","loginModule","getEbookOrderFlowV3"],function($,publicMethod,getDepositShowView,publicGetData,thirdPayMethord,ddbase,loginModule,getEbookOrderFlowV3){
  //未登录状态
  var sessionId = ddbase.token();
    if(sessionId == ''){
        loginModule();
    ...
    //获取订单信息
    function setRechargeAmount(rechargeAmount,productIds){
      var RMB = (rechargeAmount/100).toFixed(2);
      $('#getCartAllProductCnt .red_color').html('￥'+RMB);
      $('#getCartAllProductCnt').show();
      $('#quickPayment li').eq(0).attr('data-relationProductId',productIds);
      $('#quickPayment em').html(rechargeAmount);
      $('#quickPayment .rmb').html('￥'+RMB);
      $('#quickPayment').show();
    }
    var productIds = ddbase.getQueryString('productIds') || '';
    getEbookOrderFlowV3.init({productIds:productIds,callback:setRechargeAmount});

    //获取用户充值选择界面数据 paymentId:1017(微信)
    getDepositShowView.init({paymentId:1017,fromPlatform:307,el:$('#getDepositShowView'),template:$('#getDepositShowView_template').html()});
   ...
   publicGetData({url:'/media/api2.go?action=block&code=charge_mount_zx_msg',el:$('.recharge_comment .msg'),template:commentTemplate});

   //选择支付金额
   $('body').delegate('.value li','click',function(){
    var domEl = $(this);
    if(domEl.hasClass('current')) return;

    $('.value li.current').removeClass('current');
    domEl.addClass('current');
   })

   //选择支付方式weinxin和支付宝
   $('.methord input[name="recharge_methord"]').click(function(){
   		var domEl = $(this).parent().parent();
   		if(domEl.hasClass('current')) return;
   		$(this).checked = true;
   		$('.methord .current').removeClass('current');
   		domEl.addClass('current');
   		if($(this).attr('data-paymentid') == '1018'){
        //获取用户充值选择界面数据 paymentId:1018(支付宝)
   			getDepositShowView.init({paymentId:1018,fromPlatform:307,el:$('#getDepositShowView'),template:$('#getDepositShowView_template').html()});
   			return;
   		}
   		if($(this).attr('data-paymentid') == '1017'){
   			getDepositShowView.init({paymentId:1017,fromPlatform:307,el:$('#getDepositShowView'),template:$('#getDepositShowView_template').html()});
   			return;
   		}
   		if($(this).attr('data-paymentid') == '1018'){
   ...
   $('.methord li').click(function(){
   		var domEl = $(this);
   		var inputEl = domEl.find('input[name="recharge_methord"]');
   		if(domEl.hasClass('current')) return;
   		$('.methord .current').removeClass('current');
   		domEl.addClass('current');
   		inputEl[0].checked = true;
   		if(inputEl.attr('data-paymentid') == '1018'){
   			getDepositShowView.init({paymentId:1018,fromPlatform:307,el:$('#getDepositShowView'),template:$('#getDepositShowView_template').html()});
   			return;
   		}
   		if(inputEl.attr('data-paymentid') == '1017'){
   			getDepositShowView.init({paymentId:1017,fromPlatform:307,el:$('#getDepositShowView'),template:$('#getDepositShowView_template').html()});
   			return;
   		}
   		if(inputEl.attr('data-paymentid') == '1018'){
   ...
   //支付
   $('.recharge_button .conf_button').click(function(){
    $('.load_module').show();
    var payMethord = $('.methord input[name="recharge_methord"]:checked').attr('data-paymentid');
    var valueEl = $('.value .current');
    if(valueEl.length < 1){
      $('.load_module').hide();
      alert('请选择充值金额!');
      return;
    }
    var productIds = valueEl.attr('data-relationproductid');
    if(payMethord == '1017'){
      //weixin 充值
      thirdPayMethord({isPaperBook:false,act:'prepaid',fromPlatform:307,productIds:productIds,orderSource:77000,depositPayment:1017,callback:hideMask});
    }
    if(payMethord == '1018'){
      //alipay 充值
      thirdPayMethord({isPaperBook:false,act:'prepaid',fromPlatform:307,productIds:productIds,orderSource:77000,depositPayment:1018,callback:hideMask});
    }
   })
   //隐藏支付结果浮层
   $('.payResult .closeBtn').click(function(){
    $('.payResult').hide();
   })
});
define("recharge_methord", function(){});
