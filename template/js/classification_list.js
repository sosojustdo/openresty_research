...
define('classification_left_nav',["jquery","underscore","ddbase"],function($,_,ddbase){
	var classification_nav={};
	classification_nav.init=function(obj){
		var templ= _.template($(obj.template).html());

		$.get(obj.url+ddbase.setBaseApiParams(),function(data){
			...
			$(".classification_left_nav h4").on("click",function(){
				...
				$(this).parents(".publisher").siblings(".original_blank").find(".current").removeClass("current");
			})

			$(".third_level li").on("click",function(){
			...
			// 处理init方法下多级菜单自动伸缩
			var holder=$("[data-type="+obj.queryString+"]")
			if(holder.parents(".publication").length){//处理出版物
				if (holder.parents(".third_level").length){
				 	holder.parents(".third_level").slideDown().siblings().addClass("current").parents(".second_level").slideDown().parents(".first_level").addClass("selected");
				}else if(holder.parents(".second_level").length){
					holder.addClass("current").siblings(".third_level").slideDown().parents(".second_level").slideDown().parents(".first_level").addClass("selected");
				}else{
					holder.siblings(".second_level").slideDown().parents(".first_level").addClass("selected");
				}
			}else if(holder.parents(".original_blank").length){//处理原创书籍
				holder.parent(".second_level").slideDown();
				holder.addClass("current").siblings().removeClass("current");
				$(".for_original").show();
				$(".for_publish").hide();
			}
...
require(["jquery","underscore","publicMethod","classification_left_nav","dealPrice","publicHeaderNav","publicHeaderChildnavModule","publicHeadersearch","add_more","ddbase","publicSideCodeModule"],function($,_,publicMethod,left_nav,dealPrice,publicHeaderNav,publicHeaderChildnavModule,publicHeadersearch,addMore,ddbase,publicSideCodeModule){
    publicMethod.init();
    //菜单下拉条
    publicHeaderNav.itHover();
    publicHeaderChildnavModule.itHover();
	...
	var currentClassification="WY1";
	var currentSort="dd_sale";
	var currentOrder=0;
	var category=ddbase.getQueryString("category");
	var dimension=ddbase.getQueryString("dimension");
	...
	left_nav.init({
		url:"/media/api2.go?action=mediaCategory&channelType=ddds&start=0&end=5&level=5&channelId=10020",
	...
	// 处理tab逻辑
	var dealBar=function(dimension){
		var li=$("li[data-type="+dimension+"]")
		li.addClass("on").siblings("li").removeClass("on");
		var width=li.parent().find("li").eq(0).outerWidth();
		var left =li.offset().left-li.parent().offset().left+(li.outerWidth()-width)/2;
		$(".index_subnav_module .bar").width(width).animate({'left':left+'px'},100)
		currentSort=dimension;
	}

	var addMore_book=new addMore();
	...
	addMore_book.init({
		...
		eventName:'scroll',
		distance:100,
		url:"/media/api.go?action=mediaCategoryLeaf",
        dealMethod:{dealPrice:dealPrice},
		otherParam:{
			category:currentClassification,
			dimension:currentSort
		}
	})

	$("body").on("click",".classification_left_nav",function(e){
		if($(e.target).hasClass("whole")) return false;
		if(currentClassification !=$(e.target).data("type") && currentClassification!=null){
			$("#book_list").html('<div class="loading"></div>');
			currentClassification=$(e.target).data("type");
			addMore_book.init({
				template:"#classification_book_list",
				container:"#book_list",
				startNumber:0,
	...
	$("body").on("click",".publication",function(e){
		$(".index_subnav_module .for_publish").show();
		$(".index_subnav_module .for_original").hide();
		if (currentSort =="dd_sale"||currentSort =="comment"||currentSort =="newest"||currentSort =="price"){
			contentGet(e)
		}else if(currentSort =="sale"||currentSort =="comment_star"||currentSort =="update"||currentSort =="rewards"){
			dealBar("dd_sale")
			contentGet(e)
		}
	...
	$("body").on("click",".original_blank",function(e){
		$(".index_subnav_module .for_publish").hide();
		$(".index_subnav_module .for_original").show();
	...
	$(".index_subnav_module li").click(function(e){
		...
		if ($(this).hasClass("price_holder")){
			...
			$(".price_holder").removeClass("up").removeClass("down");
			currentOrder=0;
			star=true
		}
		
		if((currentSort !=$(e.target).data("type") && currentSort!=null) || star==false){
			$("#book_list").html('<div class="loading"></div>');
			dealBar($(e.target).data("type"))
			// currentSort=$(e.target).data("type");
			addMore_book.init({
				template:"#classification_book_list",
				container:"#book_list",
				startNumber:0,
