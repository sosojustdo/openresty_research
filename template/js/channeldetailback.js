...
require(["jquery","underscore","publicMethod","ddbase","loginModule",'share_module'], function ($,_,publicMethod,ddbase,loginModule,share) {
    publicMethod.init();
    var channelDetail={
        ...
        getUrl:{
            ...
            articalUrl:"/media/api2.go?action=channelArticle&cId="+ddbase.getQueryString("cId")+ddbase.baseApiParams,
            booklistUrl:"/media/api.go?action=bookListDetail&start=0&end=19&bookListId=390"+ddbase.baseApiParams
        },
        mark:{
            hasBookList:true,
            hasArticalList:true,
            ...
            isBookList:ddbase.getQueryString("booklist")
        },
        setIntroCell:function(data){
            var codeStatus=data.status.code;
            if(codeStatus===0){
                var dataObj=data.data;
                channelDetail.mark.hasBookList=!!dataObj.channel.bookList;
                channelDetail.mark.hasArticalList=!!dataObj.channel.hasArtical;
                var introCell=$(".channeldetail .channeldetail_intro")
                var channeldetailIntroTemplate=_.template($("#channeldetail_intro_module").html());
                introCell.append(channeldetailIntroTemplate(dataObj));
                if(channelDetail.mark.hasBookList && !channelDetail.mark.isBookList){
                    var bookCell=$(".channeldetail .channeldetail_book");
                    var channeldetailBookTemplate=_.template($("#channeldetail_book_module").html());
                    bookCell.append(channeldetailBookTemplate(dataObj));
                }
                if(channelDetail.mark.isBookList){
        ...
        startGetData:function(){
            ...
            $("body").on("click",".ding",function(){
                _self.subScribe($(this),_self.mark.cId,_self.mark.custId,"nosub","alreadysub","cancelsub")
            });
...
define("channeldetailback", function(){});
