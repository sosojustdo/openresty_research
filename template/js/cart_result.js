...
require(["jquery","ddbase","publicMethod","bookSixColModule"],function($,ddbase,publicMethod,bookSixColModule){
    //通用方法
    publicMethod.init();

    //取得支付铃铛数
    var payValue = ddbase.getQueryString('payValue');
    if(payValue!= null){
        $('.cart_result_container .result_detail em').html(payValue + ' 铃铛');
    }

    //倒计时
    var timerEl = $('.cart_result_container .timer span');
    var secondNum = parseInt(timerEl.html());
    setInterval(function(){
    	if(secondNum <= 1){
    		window.location.href = "booksshelf_page.html";
    		return;
    	}else{
    		timerEl.html(--secondNum);
    	}
    },1000);
