...
define('reading_center_person_info',["jquery","ddbase","loginModule"],function($,ddbase,loginModule){
	var element={};
	element.init= function(obj){
		this.checkToken(obj.tokenKey);
		this.getData(obj.url,obj.levelInfoUrl)
	};

	element.checkToken=function(key){
		var getCookie=ddbase.getCookie(key)
		if (!getCookie || getCookie == undefined){
			loginModule();
		}
	}

	element.getData=function(url,levelInfoUrl ){
		var cookieValue=ddbase.getCookie("MDD_token");
		$.get(url+"&token="+cookieValue,function(data){
			if(data.status.code == "10003"){
				loginModule();
			}else{
				var info=data.data.userInfo;
				$("#nickname_all").text(info.nickNameAll);
				$("#goldNum").text(info.goldNum);
				$("#silverNum").text(info.silverNum);
				$("#myLevel").text(info.level);
				$("#myScore").text(info.integral);
				$("#myExperience").text(info.experience);
				if(info.custImg && info.custImg!= undefined){
					$(".left_head img").attr("src",info.custImg);
				}
			}
		})

		$.get(levelInfoUrl+"&token="+cookieValue,function(data){
			if(data.status.code == "10003"){
				loginModule();
			}else{
				var bandInfo=data.data;
				$("#currentGrade").text(bandInfo.currentGrade);
				$("#nextGrade").text("LV"+bandInfo.nextGrade);

				var accountExperience=bandInfo.accountExperienceTotal
					,nextExperience=bandInfo.nextExperience;
				
				if (bandInfo.nextGrade && bandInfo.nextGrade!=0 && bandInfo.nextExperience ==0){
					$("#nextGrade").hide();
					$(".progress em").animate({"width":"128px"})
				}else{
					var progress_width=(accountExperience/nextExperience).toFixed(2)*128;
					$(".progress em").animate({"width":progress_width+"px"});
				}
				$("#currentExperience").text(bandInfo.accountExperienceTotal);
				$("#nextExperience").text("/"+bandInfo.nextExperience);
			}
		})
	}
...
require(["underscore","ddbase","reading_center_nav","reading_center_person_info","publicMethod"],function(_,ddbase,leftNav,personalInfo,publicMethod){
    publicMethod.init();
	

	leftNav.init();

	var url="/media/api2.go?action=getUser"+ddbase.setBaseApiParams()+"&selfType=0";
	var levelInfoUrl="/media/api2.go?action=getExperienceItemList"+ddbase.setBaseApiParams()+"&pageSize=20";
	personalInfo.init({
		tokenKey:"MDD_token",
		url:url,
		levelInfoUrl:levelInfoUrl
	});
});
define("reading_center", function(){});
