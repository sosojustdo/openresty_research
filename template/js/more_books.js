...
require(['jquery','underscore','publicMethod','ddbase'], function ($,_,publicMethod,ddbase) {
    publicMethod.init();

    //获取更多书籍
    var barId = ddbase.getQueryString('barId');
    var barName = ddbase.getQueryString('barName');
    var getBooks = function(){
    	var container = $('.booklist');
        container.append('<div class="loading"></div>');
    	$.get('/media/api.go?action=queryBarBook&barId='+barId,function(response){
    		container.find('.loading').remove();
    		if(response.status.code != 0) return;
    		var html = ddbase.template($('#moreBookTemplate').html(),_.extend(response.data,{getBookUrl:ddbase.getBookUrl}));
   			container.append(html);
	    })
    }
    //初始化页面
   	var html = ddbase.template($('#morebooksTemplate').html(),{title:barName});
   	$('#mainContent').html(html);
   	getBooks();
});
define("more_books", function(){});
