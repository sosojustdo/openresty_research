...
require(['jquery','underscore','ddbase',"publicMethod","dealPrice","add_more"], function ($,_,ddbase,publicMethod,dealPrice,addMore) {
    publicMethod.init();
    var moreList={
        setTitle:function(){
            //var moreListTitle=$(".morelist_title_module span");
            //var moreListTitleTxt=ddbase.getQueryString("title");
            //moreListTitle.text(moreListTitleTxt);
            //$("title").text(moreListTitleTxt);
        },
        //all_rec_xssf
        getUrl:{
            bookList:'/media/api.go?action=column&columnType='+encodeURIComponent(ddbase.getQueryString("columnType"))+'&isFull=1&returnType=json'+ddbase.baseApiParams,
            limitedfree:'/media/api.go?action=freeforlimited&columnType=rec_limited_free&channelType=all'+ddbase.baseApiParams,
            freeforlimited:'/media/api.go?action=freeforlimited&columnType=pub_limited_free&channelType=all'+ddbase.baseApiParams,
            rec_borrow_free:'/media/api2.go?action=borrow&columnType=rec_borrow_free&channelType=all'+ddbase.baseApiParams
        },
        startGetData:function(){
            var _self=this;
            _self.setTitle();
            var addMoreUrl=moreList.getUrl.bookList;

            if(ddbase.getQueryString("columnType")==="rec_limited_free"){
                addMoreUrl=moreList.getUrl.limitedfree;
            }else if(ddbase.getQueryString("columnType")==="pub_limited_free"){
                addMoreUrl=moreList.getUrl.freeforlimited;
            }else if(ddbase.getQueryString("columnType")==="rec_borrow_free"){
                addMoreUrl=moreList.getUrl.rec_borrow_free;
            }
            var addMore1=new addMore();
            addMore1.init({
                template:"#morelist_smallcell_template",
                container:".morelist .listcontent",
                startNumber:0,
                endNumber:19,
                gapNumber:20,
                eventName:'scroll',
                ...
                endCallback:function(){
                    $(".morelist .listcontent").append("<div class='go_for_more'><a href='javascript:;'>加载完毕！</a></div>");
                }
            });
    ...
    moreList.startGetData();
});
define("morelist", function(){});
