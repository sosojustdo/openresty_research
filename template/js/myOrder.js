...
;
define('addMoreOrder',["jquery","underscore","dealPrice"],function($,_,dealPrice){
	var element={};
	element.dealParam=function(obj){
		...
		this.containerWrap=obj.containerWrap;
	...
	element.getData=function(url_param){
		...
		$.get(dataUrl,url_param,function(data){
			...
			if(self.first){
				$(self.containerWrap).html('<table class="my_order_table"><tbody><tr><th>封面</th><th>书名</th><th>作者</th><th class="tx_center">简介</th><th></th></tr></tbody></table>')
				
				if($(".my_order_table").length>0){
					$(self.container).find(".loading").remove();
...
require(["jquery","underscore","ddbase","reading_center_nav","publicMethod","addMoreOrder","loginModule"],function($,_,ddbase,leftNav,publicMethod,addMore,loginModule){
   
	 publicMethod.init();
	
	//左侧菜单
	leftNav.init('#leftMyOrder');


	var order = {
		data:{
			url:'/media/api2.go?action=getMyBoughtList&fromPaltform=ds',
			detailUrl :'/media/api2.go?action=getMyBoughtDetailList&boughtId=',
			boughtId : ''
		},
		init:function(){
			var self = this;
			...
			self.showOrder();
			$('.my_order').on('click','.detail_btn',function(){
				order.data.boughtId = $(this).attr('boughtId');
				self.showDetail();
			})
		},
		showDetail:function(){
			var self = this;
			self.ajaxFn('Get',self.data.detailUrl+self.data.boughtId,{},self.showDetialSuc,self.showDetialError);

		},
		showOrder:function(){
			var self = this;
			var addMoreOrder=new addMore();
		    addMoreOrder.init({
		        template:"#myOrder",
		        container:".my_order_table tbody",
		        containerWrap:".my_order",
		        startNumber:0,
		        endNumber:9,
		        gapNumber:10,
		        eventName:'scroll',
		        distance:400,
		        url:self.data.url+ddbase.setBaseApiParams(),
		        dealMethod:{},
		        midCallback:function(){
		            $(".loading").remove();
		            $(".my_order").append($('<div class="loading"></div>'));
		        },
		        endCallback:function(){
		            $(".loading").remove();
		            $(".my_order").append($('<div class="endload">加载完毕!</div>'))
		        }
		...
		ajaxFn:function(type,url,data,successFn,errorFn){
			$.ajax({
				type : type,
		...
		showDetialSuc:function(response){
			var templ= _.template($('#myOrderDetial').html());
			var responseData = {data:response,ddbase:ddbase}
			$('#mask').remove();
			$('#detailPop').remove();
			$('body').append(templ(responseData));
			$('#mask').show();
			$('#detailPop').show();
			$('#detailPop').on('click',"#orderDetailCer,#cross",function(){
				$('#mask').hide();
				$('#detailPop').hide();
			})
		},
		showDetialError:function(){
			alert("接口出错了");
		}
	}
